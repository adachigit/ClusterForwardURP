...
// 获得影响此像素的附加光源数量和光源索引数组的其实位置
void GetAdditionalLightsCountAndIndicesStartIndex(InputData inputData, out int count, out int startIndex)
{
...
Light GetAdditionalLight(int startIndex, int i, float3 positionWS)
{
    int lightIndex = GetAdditionalLightIndexOfCluster(startIndex + i);
...
half4 ClusterFragmentPBR(InputData inputData, half3 albedo, half metallic, half3 specular,
    half smoothness, half occlusion, half3 emission, half alpha)
{
    BRDFData brdfData;
    InitializeBRDFData(albedo, metallic, specular, smoothness, alpha, brdfData);
...
//    return float(clusterIndex3D.z) / _ClusterCountParams.z;
